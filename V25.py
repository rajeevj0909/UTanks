...
class game_play:
    ...
    def window(self):
        ...
        self.enemy.fire_back(window,self.user,self.bullet)
        ...
        def tank_move(dt, control,value,direction,turn,enemy_turret_sprite):
            ...
            if turn=="Up":
                if self.count<-380: # Turn Right
                    ...
                    self.count=0
                    pyglet.clock.schedule_interval(tank_move, 1/60, self.enemy_sprite, self.tank_speed, "y","Right",self.enemy_turret_sprite)
            elif turn=="Right":
                if self.count>580: # Turn Down
                    pyglet.clock.unschedule (tank_move)
                    self.count=0
                    pyglet.clock.schedule_interval(tank_move, 1/60, self.enemy_sprite, self.tank_speed, "x","Down",self.enemy_turret_sprite)
            elif turn=="Down":
                if self.count>380: # Turn Left
                    pyglet.clock.unschedule (tank_move)
                    self.count=0
                    pyglet.clock.schedule_interval(tank_move, 1/60, self.enemy_sprite, -1*self.tank_speed, "y","Left",self.enemy_turret_sprite)
            elif turn=="Left":
                if self.count<-580: #Turn Up
                    pyglet.clock.unschedule (tank_move)
                    self.count=0
                    pyglet.clock.schedule_interval(tank_move, 1/60, self.enemy_sprite, -1*self.tank_speed, "x","Up",self.enemy_turret_sprite)

        pyglet.clock.schedule_interval(tank_move, 1/60, self.enemy_sprite, self.tank_speed, "y","Right",self.enemy_turret_sprite)

    def fire_back(self,window,user,bullet):
        def fire(dt):
            current_bullet = bullet.shoot()
            positions = user.get_position()
            current_bullet.x = self.enemy_sprite.x
            current_bullet.y = self.enemy_sprite.y
            bullet_x = current_bullet.x
            bullet_y = current_bullet.y
            tank_sprite_x = positions[0]
            tank_sprite_y = positions[1]
