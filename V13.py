    ...
    def load_tank(self):
        ...
        def on_text_motion(motion):
            if motion == pyglet.window.key.MOTION_DOWN:
                #Allows the movement if the tank's centre is above the start of the window
                ...
                #Allows the movement if the tank's centre is below the top of the window
                if self.tank_sprite.y<=window_h-(self.tank_image.height/2):
                    self.tank_sprite.y += self.steps
                    self.tank_sprite.rotation=0
            elif (motion == pyglet.window.key.MOTION_LEFT):
                #Allows the movement if the tank's centre is inside the left of the window
                if self.tank_sprite.x>=(self.tank_image.width/2):
                    self.tank_sprite.x -= self.steps
                    self.tank_sprite.rotation=270
            elif (motion == pyglet.window.key.MOTION_RIGHT):
                #Allows the movement if the tank's centre is inside the right of the window
