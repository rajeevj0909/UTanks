...
class Menu:
    ...
    def window(self):
        ...
        def on_mouse_motion(x, y, dx, dy):
            #Checks if the cursor is in the area where the buttons are
...
class User:
    ...
    def load_tank(self, window, bullet, enemy, user_health, score):
        ...
        def on_text_motion(motion):
            # Checks if there is a collision with the wall, undoes the step
        ...
        def on_mouse_motion(x, y, dx, dy):
            ...
            #If the cursor is on the left side of the tank
                ...
                #Sets rotation
                self.turret_sprite.rotation = angle
            #When mouse is vertical with the tank do nothing
            else:
                pass
...
class Bullet:
    def __init__(self):
        #User bullet
        self.bullet_image = pyglet.resource.image("bullet1.png")
        self.bullet_image.width = 22
        ...
        #Enemy bullet
        self.bullet_image2 = pyglet.resource.image("bullet2.png")
        self.bullet_image2.width = 22
        self.bullet_image2.height = 22
        self.bullet_image2.anchor_x = self.bullet_image.width / 2
        self.bullet_image2.anchor_y = self.bullet_image.height / 2
        self.speed = 500
    ...
    def shoot(self, bullet_type):
        ...
        self.bullet_no += 1
        #If the user has shot a bullet
        if bullet_type == "green":
            #Adds a new user bullet to the list to use
            self.bullets.append(pyglet.sprite.Sprite(self.bullet_image, -50, -50, batch=self.bullet_batch))
        else:
            #Adds a new enemy bullet to the list to use
            self.bullets.append(pyglet.sprite.Sprite(self.bullet_image2, -50, -50, batch=self.bullet_batch))
        #Draws all the bullets
        self.bullet_batch.draw()
        #Gives the single bullet back to use
    ...
    def load_bullet(self, window, user, enemy, leaderboard, user_health, score):
        ...
        def on_mouse_press(x, y, button, modifiers):
            ...
            if button == mouse.LEFT:
                #Draws a new bullet for use by the user
                current_bullet = Bullet.shoot(self,"green")
...
class Enemy:
    ...
    def load_enemy(self, window, user, bullet, user_health, score):
        ...
        # Turret rotation for evey enemy, same method user turret rotation
        for number in range(4):
    ...
    def move_enemy(self, window, user, bullet, enemy_sprite, enemy_turret_sprite, number, user_health, score):
        ...
        def tank_move(dt, enemy_sprite, value, direction, enemy_turret_sprite, count_x, count_y, number):
            ...
            #If the next movement is horizontal
            elif direction == "x":
    ...
    def fire_back(self, window, user, bullet, enemy_sprite, enemy_turret_sprite, leaderboard, user_health, score):
        ...
        def fire(dt, tank_sprite):
            ...
            if enemy_sprite.visible == True:
                #Get a bullet
                current_bullet = bullet.shoot("red")
                #Get the rotation
                positions = user.get_position()
            ...
            for number in range(4):
                ...
                if (bullet_sprite_y < (enemy_sprite.y + (self.enemy_image.height / 2))) and (
                    ...
                    if self.score % 40 == 0 and self.score > 0:
                        #Give the player a chance to rest before next level
                        time.sleep(1)
        ...
        # If bullet is shot from enemy
            ...
            if (bullet_sprite_y < (tank_sprite.y + (tank_image.height / 2))) and (
                ...
                if self.health < 1:
                    ...
                    bullet_sprite.visible = False
                    window.close()
                    # Asks for username function
                    def ask_username():
                        top = Tk()
                        #Creates a label for window
                        Label1 = Label(top, text=" Enter User Name, letters under 15 characters")
                        #Creates textbox
                        textbox = Entry(top, bd=5, textvariable=StringVar())
                        def callback():
                            #Sets the username as whatever textbox submits
                            global username
                        ...
                        #Places label to left and creates window
                        Label1.pack(side=LEFT)
                        textbox.pack()
                        submit.pack()
                        top.mainloop()

                        #Checks if username is a space, integer or too long #Validation
                        if username==" " or username.isdigit()==True or len(username)>15:
                            #Uses recursion to ask username again
                            ask_username()

                    #Asks username for first time
                    ask_username()
                    # Adds score and name to file
                    file = open("Leaderboard.txt", "a")
                    #Separates username and score
                    file.write(str(self.score) + "@@" + username)
                    file.write("\n")
                    file.close()
                    # Shows leaderboard at end of game
