...
class enemy:
    ...
    def move_enemy(self,window,user,bullet,enemy_sprite,enemy_turret_sprite,number):
        #Saves these points to calculate distance later on.
        ...
        def tank_move(dt, enemy_sprite,value,direction,enemy_turret_sprite,count_x,count_y,number):
            #Checks to see if tank moving is at the top or bottom
            if number % 2 == 0:
                start=0
            else:
                start=580
            #Checks to see if the horizontal movement is finished
            if direction == "y":
            ...
            # Checks to see if the vertical movement is finished
            elif direction == "x":
