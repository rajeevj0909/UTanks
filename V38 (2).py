...
class game_play:
    ...
    def window(self):
        ...
        pyglet.clock.schedule_interval(self.update_health, 1 / 60.0)
        pyglet.clock.schedule_interval(self.update_score, 1 / 60.0)
        ...
        def on_draw():
            ...
            #Draws and updates them onto the window
            health1.draw()
            health2.draw()
            score1.draw()
            score2.draw()
        ...
        if bullet_origin=="user":
            for number in range(4):
                ...
                if (bullet_sprite_y < (enemy_sprite.y + (self.enemy_image.height / 2))) and (bullet_sprite_y > (enemy_sprite.y - (self.enemy_image.height / 2)))and (bullet_sprite_x < (enemy_sprite.x + (self.enemy_image.width/ 2))) and (bullet_sprite_x > (enemy_sprite.x - (self.enemy_image.width/ 2)))and (enemy_sprite.visible==True):
                    ...
                    if self.score==40:
                        self.__init__()
                        pyglet.clock.unschedule(self.move_enemy)
                        pyglet.clock.unschedule(self.fire_back)
                        ...
                        for number in range(4):
                            enemy_sprite = enemies[number]
                            enemy_turret_sprite = enemy_turrets[number]
                            self.move_enemy(window, user, bullet, enemy_sprite, enemy_turret_sprite,
                                                  number, user_health, score)
                            self.fire_back(window, user, bullet, enemy_sprite, enemy_turret_sprite,
                                                 leaderboard, user_health, score)
