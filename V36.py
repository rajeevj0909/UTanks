...
class game_play:
    ...
    def window(self):
        ...
        def on_draw():
            ...
            self.bullet.load_bullet(window,self.user,self.enemy,self.leaderboard)
...
class bullet:
    ...
    def load_bullet(self,window,user,enemy,leaderboard):
        self.shoot()
        #Shooting
        @window.event
        def on_mouse_press(x, y, button, modifiers):
            if button == mouse.LEFT:
                ...
                def update(dt):
                    ...
                    enemy.collision(position_array,user,"user",user.user_health,window,leaderboard)
...
class enemy:
    ...
    def fire_back(self,window,user,bullet,enemy_sprite,enemy_turret_sprite,leaderboard):
        ...
        def fire(dt,tank_sprite):
            if enemy_sprite.visible==True:
                ...
                def update(dt):
                    ...
                    self.collision(position_array,user,"enemy",user.user_health,window,leaderboard)
                pyglet.clock.schedule_interval(update, 1 / 60.0)
        pyglet.clock.schedule_interval(fire, self.random_int,tank_sprite)

    def collision(self,positions,user,bullet_origin,user_health,window,leaderboard):
