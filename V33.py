...
class menu:
    ...
    def window(self):
        ...
        self.menu_sprite = pyglet.sprite.Sprite(self.menu)

        @window_menu.event #Add the image to the window
        def on_draw():
            window_menu.clear()
...
class user:
    ...
    def update_user_health(self,user_health):
        ...
        def on_mouse_press(x, y, button, modifiers):
            if button == mouse.LEFT:
                ...
                def update(dt):
                    ...
                    enemy.collision(position_array,user,"user",user.user_health,window)
        ...
        def fire(dt,tank_sprite):
            if enemy_sprite.visible==True:
                ...
                def update(dt):
                    ...
                    self.collision(position_array,user,"enemy",user.user_health,window)
                pyglet.clock.schedule_interval(update, 1 / 60.0)
        pyglet.clock.schedule_interval(fire, self.random_int,tank_sprite)

    def collision(self,positions,user,bullet_origin,user_health,window):
        ...
        if bullet_origin=="user":
            for number in range(4):
                ...
                enemy_turret_sprite = self.enemy_turrets[number]
                if (bullet_sprite_y < (enemy_sprite.y + (self.enemy_image.height / 4))) and (bullet_sprite_y > (enemy_sprite.y - (self.enemy_image.height / 4)))and (bullet_sprite_x < (enemy_sprite.x + (self.enemy_image.width/ 4))) and (bullet_sprite_x > (enemy_sprite.x - (self.enemy_image.width/ 4)))and (enemy_sprite.visible==True):
            ...
            if (bullet_sprite_y < (tank_sprite.y + (tank_image.height / 2))) and (bullet_sprite_y > (tank_sprite.y - (tank_image.height / 2))) and (bullet_sprite_x < (tank_sprite.x + (tank_image.width / 2))) and (bullet_sprite_x > (tank_sprite.x - (tank_image.width / 2))):
                ...
                user.update_user_health(user_health)
                if user_health<1:
                    user.update_user_health(0)
