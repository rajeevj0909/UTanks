    ...
    def load_bullet(self):
        ...
        def on_mouse_press(x, y, button, modifiers):
            if button == mouse.RIGHT:
                ...
                #Bullet movement
                ###USED NEW #Using https://pyglet.readthedocs.io/en/pyglet-1.3-maintenance/programming_guide/examplegame.html

                #If first bullet
                if self.count==0:
                    #Runs an update for the movement
                    def update1(dt):
                        ...
                        print(self.count)
                    pyglet.clock.schedule_interval(update1, 1 / 60.0)
                #If not first bullet
                elif self.count>0:
                    pyglet.clock.unschedule(self.update1)
                    dt=0
                    def update2(dt):
                        rad = math.radians(self.bullet_sprite.rotation)
                        cosAngle = math.cos(rad)
                        ...
                        self.bullet_sprite.y += cosAngle * 500 *dt
                        self.bullet_sprite.x += sinAngle * 500 *dt
                    pyglet.clock.schedule_interval(update2, 1 / 60.0)
