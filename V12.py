...
class enemy:
    def __init__(self):
        ...
        self.enemy_image.anchor_y = self.enemy_image.height / 2     #It will be placed at the bottom left
        self.enemy_sprite = pyglet.sprite.Sprite(self.enemy_image, x=window_w / 5, y=window_h / 5)

        #Enemy turret is defined ready for later use.
        ...
        #Enemy bullet is defined ready for use if needed
...
#Pushed this to outside both classes for objects to run from both
@window.event
def on_draw():
    window.clear()
