...
class game:
    ...
    #Gets called by collision function from enemy class
    def get_position(self):
        #Turns positions into a list
        position_array=[self.bullet_sprite.x,self.bullet_sprite.y]
        #Sends positions back to collision function
...
class enemy:
    ...
    #Runs when bullet is fired
    def collision(self):
        #Takes a list of positions from game class
        positions=bullet_object.get_position()
        #Names the items in the list
        bullet_sprite_x=positions[0]
        bullet_sprite_y=positions[1]
        #Checks if the positions are currectly output.
        print(positions)
