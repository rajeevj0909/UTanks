...
class leaderboard:
    ...
    def leaderboard_window(self):
        ...
        file.write(name+"@@"+score)#Adds name and score
        file.write("\n")#With a new line
        file.close()
        # Opens in write mode
        self.leaderboard_list=[]
        ...
        #Reads each line of the column
        for line in column:
            #Splits the people from each other
            line = line.split("\n")
            #Deletes the space at the end of the line
            line.pop(1)
            # Divides the name from the score
            separated=line[0].split("@@")
            #Adds the name and score list to the leaderboard list
            self.leaderboard_list.append(separated)#Creates 2D array
        file.close()
